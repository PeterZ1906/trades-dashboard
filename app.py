...
def next_tid(df: pd.DataFrame) -> int:
    if df.empty or df[TID].isna().all():
        return 1
...
def add_trade_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[df["realized"] & (df["entry_total"] > 0), "ret_pct"] = (
        df.loc[df["realized"], "pnl"] / df.loc[df["realized"], "entry_total"] * 100
    )
...
def card(title: str, value_html: str, badge: str | None = None, color: str | None = None):
    badge_html = (
        f'<span class="badge" style="border-color:{color or "rgba(255,255,255,.18)"};color:{color or "#e5e7eb"}">{badge}</span>'
        if badge
        else ""
    )
    st.markdown(
...
def build_stocks_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pretty stock label
    d["Stock (Company • Ticker)"] = d.apply(
        lambda r: f"{(str(r['company']).strip() if str(r['company']).strip() else r['symbol'])} • {r['symbol']}", axis=1
    )
    ...
    # Order by Symbol for a stable, readable layout
    out = out.sort_values(["Symbol"], ascending=True).reset_index(drop=True)
    return out


# NOTE: removed the problematic type hint that caused AttributeError on some pandas builds
...
if page == "Dashboard":
    st.markdown(
        f"## Portfolio Dashboard — <span style='color:{ACCENT2}'>{theme_name}</span>",
        unsafe_allow_html=True,
    )
    ...
    if df.empty:
        ...
        if sel_syms:
            df_f = df_f[df_f["symbol"].isin(sel_syms)]
        ...
        with c1:
            card("Portfolio Value*", mask_money(total_invested + realized_pnl), "proxy", ACCENT2)
        with c2:
            card("Open Cost", mask_money(open_cost))
        with c3:
            card("Realized P&L", mask_money(realized_pnl), "realized", GOOD if realized_pnl >= 0 else BAD)
        with c4:
            card("Win Rate", mask_pct(win_rate), "realized", GOOD)
        with c5:
            card("Avg Return / Trade", mask_pct(avg_ret), "realized", GOOD)
        ...
        with tab4:
            ...
            for c in ["entry_total", "exit_price", "fees_total", "pnl"]:
                if c in t.columns:
                    t[c] = t[c].map(lambda x: mask_money(float(x)))
            if "ret_pct" in t.columns:
                t["ret_pct"] = t["ret_pct"].map(lambda x: mask_pct(float(x)))
            if "shares" in t.columns:
                t["shares"] = t["shares"].map(lambda x: f"{float(x):,.4f}")
            if "hold_days" in t.columns:
                t["hold_days"] = t["hold_days"].map(lambda x: "" if pd.isna(x) else int(x))
            ...
            if stock_df.empty:
                ...
                # Robust render: use HTML to ensure Styler works across versions
                try:
                    html = styled.hide_index().to_html()
                except Exception:
                    # .hide_index missing on very old pandas
                    html = styled.to_html()
...
# ============================================================
    ...
    if df.empty:
        ...
        with c1:
            valid = df[~df["entry_dt"].isna()]
            dmin, dmax = (
                (valid["entry_dt"].min().date(), valid["entry_dt"].max().date()) if not valid.empty else (date.today(), date.today())
            )
        ...
        with k1:
            card("Realized Net Gain/Loss", mask_money(realized_net), color=GOOD if realized_net >= 0 else BAD, badge="realized")
        with k2:
            card("Portfolio Net Gain/Loss", mask_money(realized_net), color=GOOD if realized_net >= 0 else BAD)
        with k3:
            card("Winners", f"{winners}")
        with k4:
            card("Losers", f"{losers}")
        with k5:
            card("Average Win", mask_money(avg_win))
        with k6:
            card("Average Loss", mask_money(avg_loss))
        ...
        if realized.empty:
            ...
            summ.rename(
                columns={"pnl": "pnl($)", "avg_ret": "avg_return(%)", "avg_hold": "avg_hold(days)"},
                inplace=True,
            )
...
# ============================================================
    ...
    if df_all.empty:
        ...
        df_show = df_all[
            [TID, "symbol", "side", "shares", "entry_date", "entry_total", "exit_date", "exit_price", "fees_total", "strategy", "notes", "created_at"]
        ...
        def label_row(r):
            return f"#{int(r[TID])} · {r['symbol']} · {r['side']} · {r['shares']} · {r['entry_date']}"
        ...
        with st.form("edit_trade_form", enter_to_submit=False):
            ...
            if b2.form_submit_button("Close Trade"):
                if float(e_exit_price) <= 0:
                    st.error("Set an Exit Price to close.")
                else:
            ...
            if b4.form_submit_button("Delete This Trade"):
                save_trades(df_all[df_all[TID] != edit_tid].copy())
